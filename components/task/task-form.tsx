...
const deriveStatusFromProgress = (progress: number, isBlocked: boolean): TaskStatus => {
...
interface TaskFormProps {
  ...
  tasks: Task[]
  onSave: (task: Omit<Task, "id"> | Task, resourceAssignments?: Omit<NewResourceAssignment, "task_id">[]) => void
  onCancel: () => void
}
...
export function TaskForm({
  ...
  const defaultAssigneeId = task?.assignee_id || (teamMembers.length > 0 ? teamMembers[0].id : "")
  ...
  useEffect(() => {
    ...
    for (const selected of selectedResources) {
      if (!selected.start_date || !selected.end_date || selected.resource_id === "") continue
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (new Date(formData.start_date) > new Date(formData.end_date)) {
      setCriticalError("La fecha de inicio debe ser anterior o igual a la fecha de fin.")
      return
    }
    ...
    if (!formData.assignee_id || !teamMembers.some((m) => m.id === formData.assignee_id)) {
    ...
    for (const resource of selectedResources) {
      if (
        ...
        setCriticalError("Todas las asignaciones de recursos deben tener fechas de inicio y fin válidas.")
        return
      }
      if (resource.start_date && resource.end_date && new Date(resource.start_date) > new Date(resource.end_date)) {
        setCriticalError("La fecha de inicio de la asignación debe ser anterior a la fecha de fin.")
        return
      }
      if (
        (resource.start_date && new Date(resource.start_date) < new Date(formData.start_date)) ||
        (resource.end_date && new Date(resource.end_date) > new Date(formData.end_date))
      ) {
        setCriticalError("Las fechas de asignación de recursos deben estar dentro de las fechas de la tarea.")
        return
      }
    }
    ...
    const validResources: Omit<NewResourceAssignment, "task_id">[] = selectedResources
      .filter((r) => r.resource_id !== "" && r.hours_assigned > 0 && r.start_date && r.end_date)
      .map((r) => ({
    ...
    >
        ...
        >
          <div className="flex items-start gap-2 md:gap-3">
            ...
            <div className="flex-1">
              <h4 className="font-semibold text-red-600 text-sm md:text-base">Proyecto Cerrado</h4>
      ...
      >
        ...
        <select
          ...
          onChange={(e) => setFormData({ ...formData, assignee_id: e.target.value as UUID })}
          disabled={isDisabled}
        >
          {teamMembers.length === 0 && <option value="">No hay miembros de equipo</option>}
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <label
            htmlFor="blocked"
            className={`text-sm text-foreground cursor-pointer ${formData.progress === 100 ? "opacity-50" : ""}`}
          >
        ...
        {selectedResources.length > 0 && (
          <div className="space-y-3 border border-border rounded-lg p-2 md:p-3 bg-muted/30">
            {selectedResources.map((selected, index) => (
              <div key={index} className="space-y-2 p-2 md:p-3 bg-background rounded-md border border-border">
                <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-start">
                  <div className="flex-1">
                    ...
                    <select
                      ...
                      onChange={(e) => handleResourceChange(index, "resource_id", e.target.value as UUID)}
                      disabled={isDisabled}
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        handleResourceChange(index, "start_date", e.target.value)
                      }}
                      min={formData.start_date || project.start_date}
                      max={selected.end_date || formData.end_date || project.end_date}
                      className="text-sm"
                      disabled={isDisabled}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      min={selected.start_date || formData.start_date || project.start_date}
      ...
      {criticalError && (
        ...
        >
          <div className="flex items-start gap-2 md:gap-3">
            <AlertTriangleIcon className="h-5 w-5 text-red-600 shrink-0 mt-0.5" />
            <div className="flex-1">
              <h4 className="font-semibold text-red-600 text-sm md:text-base">Error de Validación</h4>
              <p className="text-xs md:text-sm text-red-700 mt-1">{criticalError}</p>
            </div>
          </div>
        </motion.div>
      )}
      ...
      {resourceConflicts.length > 0 && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
        >
          <h4 className="font-semibold text-yellow-600 mb-2 text-sm md:text-base">Conflictos de Recursos Detectados</h4>
          <ul className="list-disc list-inside text-xs md:text-sm text-yellow-700 space-y-1">
            {resourceConflicts.map((conflict, i) => (
              <li key={i}>
                <strong>{conflict.resourceName}</strong> asignado de {conflict.dates}. Conflictos:{" "}
