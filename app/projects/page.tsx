...
    return resourceAssignments.filter((a) =>
      projectTasks.some((t) => t.id === a.task_id)
    );
  }, [resourceAssignments, projectTasks]);
...
    const savedProject = await ("id" in projectData ? updateProject(projectData as Project) : createProject(projectData));
    if (savedProject) {
...
    if (confirm(`¿Estás seguro de que quieres eliminar "${selectedProject.name}"? Esto también eliminará tareas y asignaciones.`)) {
...
    const savedTask = await ("id" in dataToSave ? updateTask(dataToSave as Task, resourceAssignments || []) : createTask(dataToSave as Task, resourceAssignments || []));
    if (savedTask) {
  ...
  if (!selectedProject) {
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col gap-4 items-center justify-center p-8">
        <p className="text-xl">No hay <strong>proyectos</strong> seleccionados.</p>
        <Button onClick={() => { setEditingProject(undefined); setIsProjectModalOpen(true); }}>
          Crear un Proyecto
        </Button>
        {/* Aquí también usamos el Dialog para el primer proyecto */}
        <Dialog open={isProjectModalOpen} onOpenChange={(isOpen) => { if (!isOpen) { setIsProjectModalOpen(false); setEditingProject(undefined); } }}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingProject ? "Editar Proyecto" : "Crear Nuevo Proyecto"}</DialogTitle>
            </DialogHeader>
            <ProjectForm project={editingProject} teamMembers={allTeamMembers} onSave={handleSaveProject} onCancel={() => { setIsProjectModalOpen(false); setEditingProject(undefined); }} />
          </DialogContent>
        </Dialog>
      </div>
    );
  ...
  return (
    <div className=" text-foreground">
      <div className="flex h-[calc(100vh-73px)] md:h-[calc(100vh-89px)] relative">
        ...
        <div className={`fixed md:relative inset-y-0 left-0 z-50 md:z-0 transform transition-transform duration-300 ease-in-out ${isMobileSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
          <ProjectList
            projects={projects}
            tasks={tasks}
            selectedProjectId={selectedProjectId || projects[0]?.id}
            onSelectProject={handleSelectProject}
            onAddProject={() => { setEditingProject(undefined); setIsProjectModalOpen(true); setIsMobileSidebarOpen(false); }}
          />
        </div>
          ...
          <div className="p-4 md:p-6">
            ...
            {activeTab === "assignments" && (
              <ResourceAssignmentList
                ...
                tasks={projectTasks}
                onAddAssignment={() => { setEditingAssignment(undefined); setIsAssignmentModalOpen(true); }}
                onEditAssignment={(a: ResourceAssignment) => { setEditingAssignment(a); setIsAssignmentModalOpen(true); }}
      ...
      <Dialog open={isProjectModalOpen} onOpenChange={(isOpen) => {
        ...
        <DialogContent>
          ...
          <ProjectForm
            ...
            onCancel={() => { setIsProjectModalOpen(false); setEditingProject(undefined); }}
          />
        </DialogContent>
      </Dialog>

      {/* 2. Modal de Tarea */}
      <Dialog open={isTaskModalOpen} onOpenChange={(isOpen) => {
